...
`A` 进入快速布局，用 Normal 的方向键调整半屏、四分之一等比例；`E` 自动布局并进入树编辑，Shift+方向分区，Ctrl+方向短按一步、长按连续移动分割线，X 删除当前分区。数字选窗，只有歧义前缀等待（默认 250ms）。A / E 都可在 Alt+W 后直接使用。所有调整即时生效，Q 从布局返回窗口模式，再按 Q 按 `window.exit_mode` 退出（默认 `return`）；Tab 直接切窗，Z 撤销。
