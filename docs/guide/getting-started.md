...
把鼠标放在要调整的窗口上，按 `Alt+W` 进入 Window 并锁定它。方向键移动窗口，`S` 切换到中心缩放；按住 `Primary` 可临时使用 Normal 的真实绑定移动鼠标和点击，松开后继续调整同一窗口。

`A` 进入快速布局，用 Normal 的方向键调整半屏、四分之一等比例；`E` 进入树编辑，Shift+方向分区，Ctrl+方向调整分割线。数字选窗，只有歧义前缀等待（默认 250ms）。所有调整即时生效，Esc 返回并保留布局，Q 退出，`AA` 一次平铺；Tab 直接切窗，Z 撤销。
