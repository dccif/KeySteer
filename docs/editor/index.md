...
已有 `window-layouts.kslayout` 时，从程序菜单打开模拟器会同时带入当前按键配置和布局列表，无需另选文件。选择布局编号后编辑分区；Ctrl+S 更新该布局的备注，或直接点“保存并下载布局”保存当前修改并下载整个布局库，保留其他布局。把下载的 `window-layouts.kslayout` 替换程序同目录的同名文件即可；每次 `Alt+W → R` 都重新读取，不需要重启、Reload Configuration 或文件监听。网页也保留“导入布局文件”作为独立入口。
