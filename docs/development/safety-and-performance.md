...
- 便携层（`api`、`app`、`config`、`modes`、`plugins`、`runtime`、`support`）保持 safe Rust；原生 FFI 集中在平台层，并由局部封装维持所有权、线程和指针不变量。
...
- Windows `SendInput` 失败日志必须保留实际返回数量和即时 `GetLastError`，并只在失败路径
  ...
  字符串或执行额外系统查询。
- Windows Engine 完成键盘 disposition 后，不得同步等待低级 Hook 线程执行 `SendInput`：
  快速的下一次物理边沿可能已经让 Hook 等待 Engine，形成双向等待。鼠标按钮、滚轮和键盘
  注入进入预分配的 32 项有界 FIFO，由 Hook 返回消息循环后串行执行；相邻请求合并唤醒，
  执行失败再异步复位输入状态。不得改成 sleep、轮询、无限队列、额外注入线程或连续移动
  热路径上的等待。
