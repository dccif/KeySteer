...
如果持续按住`鼠标按键`键达到 `long_press_toggle_ms`，鼠标会进入 `Toggle` 状态，也就是按下模式；再次长按该点击键或使用 `n + 点击键` 才释放。也可以把多个动作组合起来：
...
注意：在定位 Mode 中，Grid 标签或 UI Hint 标签可能优先于继承来的 Normal 按键；按住 `Primary` 可临时使用 Normal。
