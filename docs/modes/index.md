...
先匹配完整组合键，临时模式优先于当前模式；未命中时，再去掉临时激活键，以相同层级顺序匹配。例如完整 `primary+s` 优先于裸 `s`，Grid 的 `primary+q` 优先于 Normal 的裸 `q`。用 `temporary_mode_passthrough_keys = ["q"]` 可以让 Q 跳过临时层，交给当前模式。其他修饰键仍参与匹配。
