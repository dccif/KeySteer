...
Windows 显式选窗（含 Normal 激活动作）先调用 SetForegroundWindow；被拒绝后发送带自身注入标记和菜单屏蔽的 Alt 批次以解除前台锁，再重试。不得使用可能发送 SC_RESTORE 的 SwitchToThisWindow 回退：普通未分组窗口的数字编号和 Tab 选择只激活，不修改矩形、最大化或全屏状态；macOS 对应路径仅激活应用并 AXRaise。已按住 Alt 时不注入、不释放；部分注入失败补发屏蔽与释放，避免残留。恢复仅发生在显式激活失败路径，worker 有界回读前台，不用 AttachThreadInput 连接外部输入队列。Window 模式最终拒绝焦点时仍锁定下一窗口并提示；Normal 独立激活动作失败不移动鼠标。
...
普通窗口及组内活动成员的 Move、MoveTo、Resize 使用 `WindowAccess::submit_frame` 提交，随后由 `PendingAdjustment` 读回确认。提交复用准备阶段的快照；读回使用 `refresh_geometry` 原地复用元数据。原生事件负责唤醒，存在待确认操作时才启用最长 16ms 的读回兜底，250ms 截止；约束尺寸需稳定观察，最多一次重试及一次中心修正。只有实际读回才能报告成功／changed，部分生效仍保留撤销信息。

同一目标及其组内别名保持屏障，窗口请求不跨越更早的阻塞请求，结果按请求次序发送。每轮推进所有独立待确认窗口；后面的窗口可以先读回完成，但历史和结果仍按请求顺序提交。已完成的窗口不再轮询。确认监听与持久组监听合并，不能相互覆盖。原生 IPC 本身不保证零阻塞。Windows 几何确认不调用 DwmFlush，保留的显示同步仅用于专有绘制／捕获路径。

Windows `OwnedFont` 拥有 HFONT；选入 DC 的 guard 同时借用字体和独占绘图 surface，析构恢复旧对象，类型系统阻止选入期间释放字体或 surface。UIA 的 CachedElement 统一收拢缓存属性 unsafe 调用，返回拥有所有权的 Rust 值；COM 与 GDI 操作继续限制在原生模块。



普通窗口确认现在使用 `refresh_geometry(&mut Snapshot)` 而不是每轮返回新快照；两端原生实现不修改 title/app 存储。`can_submit_frame` 只做廉价能力判断，不调用快照读取。异步拒绝携带已有快照进入同步执行；若需等待先前事务，丢弃该临时快照并在屏障后重新准备，避免使用过期几何。

### 状态与布局事务确认

`PendingAdjustment` 包含恢复普通状态、几何、最大化及标签栏占位校正阶段。Windows 用异步原生状态提交，macOS 保留已有 work_area 模拟最大化语义；只有观测到状态和几何才结束确认。组内请求映射到活动成员，布局代表编号只用于去重；title/app 不参与每轮转换。原生回声不能在确认期间再触发同步 fit_header。

`PendingLayout` 移入请求负载，分批准备快照；每批最多 8 项或 4ms，并在每项前检查取消。完整校验结束前不能提交写入。准备、待提交、在途和完成状态分开保存，仅最多 16 个在途窗口持有完整确认状态并参与轮询。独立确认不让一个慢窗口阻止其他窗口读回；整个事务仍是后续窗口请求的屏障，音频仍可分发。提交失败也等待已提交写入确认或截止，再按提交顺序反向恢复全部尝试过的窗口；恢复写入不能因为一次旧快照与目标相同而省略。恢复失败时异步尝试编辑入口布局并结束事务。EndEdit(commit=false) 同样分批异步恢复；commit 时才记录撤销历史。无操作的正常布局不重复提交。

`transaction.rs` 统一布局和历史操作的 worker 调度。`history_confirmation.rs` 将 Undo、Redo、ResetInitial 拆成有界准备和独立确认，复用准备快照；取消后保留未完成的源历史以及已提交变化的逆操作。全屏、最小化等不支持的转换仍逐窗口使用同步兼容路径；结束时保留完整库存核对。单次原生 IPC 和最终库存读取不保证非阻塞。

标签原生事件按涉及窗口及所在组判断确认屏障，其他组可继续处理；依赖同一组的后续结构事件不能越过已延迟事件。延迟动作最多 64 项，安全合并焦点事件；溢出的关闭身份去重保留，其余溢出动作明确记录拒绝。关闭身份存储受保留的原生身份数量约束。
