...
`PendingLayout` 先复用同步路径的完整校验，再最多同时提交 16 个窗口。独立确认不让一个慢窗口阻止其他窗口读回；整个事务仍是后续窗口请求的屏障，音频仍可分发。提交失败也等待已提交写入确认或截止，再反向恢复全部尝试过的窗口；恢复写入不能因为一次旧快照与目标相同而省略。恢复失败时异步尝试编辑入口布局并结束事务。EndEdit(commit=false) 同样分批异步恢复；commit 时才记录撤销历史。无操作的正常布局不重复提交。
