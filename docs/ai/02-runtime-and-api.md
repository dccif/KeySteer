...
生产 Reload 候选可携带应用层 `PreparedRestart`，Engine 接受后停止事件循环并完成正常退出，不替换旧实例中的 Mode 集。bootstrap drop Engine/Backend 后，CLI 才提交新进程启动；校验失败保持当前实例。普通 `set_config` 仍走计划替换。
...
事件处理完成后，若长按、快捷面板、拖动释放、timer 和延迟 sequence 均无待处理状态，当前 turn 直接结束；条件检查发生在事件之后，不能跳过事件刚刚创建的任务。有任务时仍按原次序处理与恢复，延迟处理主体独立放置，避免扩大普通输入路径。关闭按键解析调试时直接跳过追踪，启用时进入独立的冷路径。
