...
- Partial 始终在去重、重新分配短标签后立即显示；按住 overlap cycle key 也不会冻结后续批次。
  扫描期间的 Shift 被忽略，不记录状态、不计算层级或额外重绘；terminal 从最终已合并标签建立
  一次轻量视觉计划。扫描结束后的 Shift 只读取缓存并逐次整体显示一个层；不同重叠区域不会
  按各自组大小独立取模；中心被遮挡或交叠达到较小标签20%才进入层计划，轻微边缘接触不会
  虚增层数。Hint前缀、Backspace和名称搜索每次改变可见集合后都会立即替换计划，Shift始终
  对当前筛选结果生效。层切换只能改变 z-index，不能改变标签数量、文字或坐标。
