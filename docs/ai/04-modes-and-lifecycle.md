...
- Partial 始终在去重、重新分配短标签后立即显示；扫描期间的 Shift 被忽略，不记录状态、
  不计算层级或额外重绘。terminal 从最终已合并标签建立视觉计划；已有 Hint 前缀时，晚到
  Partial 保留目标但不重排现有键码，前缀清空后再统一合入并重建计划。
- 视觉计划按相交连通分量复用冲突 bitset，并以绘制正序、逆序、相交度、行优先和列优先五种
  固定 first-fit 顺序择优、压低层号；仍只有中心被遮挡或交叠达到较小标签 20% 才算冲突。
  不同分量共享全局层号；松开 Shift 显示第 0 层，按下只在非默认层 `1→2→…→1` 间循环，
  因而两层场景每次按住都会显示第 1 层，不会产生一次无视觉变化的按键。Hint 前缀、
  Backspace 和名称搜索每次改变可见集合后都会重建计划。分层只修改 z-index，绝不移动、
  裁剪、删除或重新分配标签。
