...
工作区持久化复用惰性单 worker 和 16 项有界队列，满时返回明确错误，输入线程不等待磁盘锁；使用次数最多额外排队一份快照，累计值单调合并，退出时补齐最新值。没有周期刷盘定时器。禁用的日志在宏入口判断级别，连 target 和格式化参数都不求值。错误仍保留同步可靠落盘语义，因此恢复动作必须先于错误日志。
...
日志保留第一条 Error 立即写入／flush。相同 target 和完整 message 在 1s 内合并重复项，下一条不同记录、到期后的记录或退出 flush 写出次数；精确键最多 4096 字节、只缓存一个，失败 sink 不抑制重试。不开定时器、不新增日志线程，异常风暴减少重复格式化与落盘。没有增加生产性能插桩或新的探测基准。
