...
最终保留 SmolStr、独立 FxHashMap、整表别名作用域复用与 Hint placements 预留，撤回 CompactString。五轮平衡次序的五版本对照中，相对本轮 SmolStr 参考版，FxHashMap 的 21 个非初始化场景 p99 中位数全部持平或下降；100 目标扫描下降 32.2%–43.5%，setup +0.4% 按新要求允许。CompactString 的 100 目标 Hint p99 +13.3%，组合 +21.1%，该场景 A/A +0.6%，没有将这些差异直接归因到具体指令或哈希算法。证据见 `target/perf-runtime-priority/libraries-recheck.md`；五轮汇总通过不保证每轮或所有机器无波动，也不是对旧 c2d718d 的重新对照。

大规模 Hint 的 placements 缓冲在 clear 后按已知 visible 数量 reserve，避免过滤迭代器丢失精确 size_hint 后在首次扫描中逐级扩容；后续重扫仍复用已有容量，不额外缓存目标或改变重叠算法。2000 目标 owned delivery 分配门禁请求字节为 630,928，allocation 仍为 13 次。

当前验收优先运行时按键与模式响应，允许少量内存增加和一次性初始化变慢；初始化仍单独报告并尽量优化。SmolStr 单独恢复的分阶段实验保存在 `target/perf-runtime-priority/`，整表别名作用域复用与 Key 表示变更分别构建测量。下段是旧验收条件下的历史记录，旧 `perf-libraries` 的三个库组合及其撤回结论不能代替新组合的数据。

优化库按 CompactString、FxHashMap、SmolStr 顺序实验，最终全部撤回：SmolStr 七对实验 setup p99 +14.56%；前两者组合在全场景与 A/A/B 复核中仍有 Hint p99 增加，未通过严格防回退验收。A/A 本身也有波动，尚未把差异定位到具体库，不据此断言库本身必然更慢。保留 `core_hot_paths --scan-index`，调用真实 SpatialIndex，覆盖 24/100/500/2000 目标的稀疏、重复、稠密分布，计时含创建与销毁、不含 fixture 构造。它衡量 CPU 去重而非 UIA/AX 完整扫描。实验数据位于 `target/perf-libraries/`；其中 final 目录是已撤回的组合候选，不能当作当前生产性能。

Hint 空前缀直接使用候选数量；没有活动重叠层时只遍历实际绘制的 z 层，避免为不存在的层重复扫描标签。16–128 候选复用既有 inline sweep；稠密、无效矩形和稳定绘制顺序由二次参考实现对照验证。没有增加缓存、HashMap 替代哈希器或常驻分配。


Windows GPU 继续复用单一静态 surface 与独立动态层，dismiss 释放内容树；退出覆盖层时额外回收超过 4096 个 UTF-16 单元容量的异常大文本缓冲，普通缓冲继续复用。多屏拆分 surface 的实验已撤回：同一进程 A/B/A、debug 构建、每组 20,000 帧，64 标签重绘的拆屏 p99 约 1.26ms，对照约 0.65ms；纯背景拆屏中位约 205μs，对照 152–163μs，虽然后者 p99 改善，但不符合响应优先目标。这些是本机 present 调用测量，不是端到端延迟或实际 GPU 驻留/RSS 测量。

输入恢复在统一错误日志前完成。快捷切换几何查询异步，Windows 焦点等待改为 worker deadline；这些变化去掉指定调用栈上的等待，不保证跨进程原生 API 本身没有延迟，也不是端到端性能提升百分比。
...
Window 固定帮助只解析当前模式及其显式继承链，不模拟激活临时层；仅 Window Move 额外添加经过实际 resolver 校验的 Grid/Recursive Grid 入口。临时 Normal 的发送、跨屏及其他绑定不混入固定面板。


## 有界异步工作与内存复用

普通窗口几何提交后将等待交回已有 worker 事件循环；队列最多 64 项，待确认最多 16 项，不创建每请求线程。相同屏幕集合以 `Arc<[Screen]>` 在队列中共享；屏幕变化更新会话几何上下文。撤销、重做、初始状态和编辑事务使用只含标量的 PlacementSnapshot，不复制 title/app 字符串；恢复时取得当前身份元数据，不能用旧标题覆盖新标题。

工作区持久化复用惰性单 worker 和 16 项有界队列，满时返回明确错误，输入线程不等待磁盘锁；使用次数最多额外排队一份快照，累计值单调合并，退出时补齐最新值。没有周期刷盘定时器。禁用的日志在宏入口判断级别，连 target 和格式化参数都不求值。错误仍保留同步写入／flush 语义（不承诺断电持久化），因此恢复动作必须先于错误日志。

这些变化减少可确认的热路径等待和字符串复制，但并不构成端到端延迟或进程 RSS 改善比例。复杂窗口事务、原生跨进程调用、配置读取仍有同步路径；性能验收应分别测量提交、原生确认、输入到像素和稳定运行内存，不能相互替代。


## 窗口确认与 Normal 巡航的再次收紧

PendingAdjustment 只拥有一份携带 title/app 的 observed 快照和一份无字符串的 before。Windows/macOS 的 `refresh_geometry` 原地更新 observed 几何／状态，轮询不克隆 title/app，完成后将字符串所有权移入 WindowResult。known group/hidden/unsupported 目标通过 `can_submit_frame` 提前走同步路径；其他回退把已读快照交给同一轮的 execute_prepared，不能跨等待屏障缓存快照。测试锁定等待 10,000 次零分配，以及普通不支持、最大化、最小化、全屏、亚像素和原生拒绝分支不增加快照读取次数。

Normal 的 PointerSettings 在构造时转换为同尺寸 MotionProfile，用预计算 ramp_duration 替换 acceleration，不扩大模式对象、不分配额外堆内存。巡航阶段直接求线性积分，跳过重复求 ramp duration、smootherstep 及边界组合；斜向归一、亚像素、速度倍率和真实 elapsed 语义不变。验证在多组速度／加速度、平滑和线性模式下与原积分比较，并覆盖跨巡航边界及等时间不同帧率。

## 确认查询与慢窗口隔离

Windows 普通确认只验证身份、读取可见 bounds 和一次 GWL_STYLE；正常状态不再每轮读取 WINDOWPLACEMENT、扩展样式、原始外框和恢复位置。最小化／隐藏准备状态仍走完整几何路径。macOS 普通等待只读 AX position / size，完成时再校验最小化、全屏和模拟最大化状态；状态过渡仍读完整标志。AXMinimized CFString 与 AXFullScreen 一样按窗口缓存。无 title/app 复制；组内标量投影使用无元数据的临时视图。

每个独立窗口均推进确认；已就绪结果排队等待更早结果，不再重复原生查询。布局在途上限 16。此处消除了主动等待造成的队首阻塞，并不隔离单次跨进程原生查询本身的延迟。
